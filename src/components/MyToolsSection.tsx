...
function MyToolsSection() {
  ...
  const stackG = ["Docker", "Postman", "REST API"];
  ...
  return (
    <>
      <section ref={stackRef} className="max-w-5xl mx-auto pb-20">
        ...
        >
          <h3 className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl xl:text-9xl font-clash text-white">
            My Tools
          </h3>

          <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col gap-2">
            <h4 className="text-2xl sm:text-3xl md:text-4xl xl:text-5xl font-clash text-red-700">
              My Heart
            </h4>
            {coreStack.map((tech) => (
              <span
                key={tech}
                className="text-xl font-clash text-white/60 tracking-wide"
              >
                {tech}
              </span>
            ))}
      ...
      >
        ...
        >
          {[...Array(2)].map((_, j) => (
            <span
              key={j}
              className="flex gap-8 text-2xl sm:text-4xl md:text-6xl font-clash text-white uppercase tracking-widest"
            >
              {stack.map((tech) => (
